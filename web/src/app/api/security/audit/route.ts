  ...
  try {
    ...
    await prisma.securityAudit.create({
      data: {
        ...
        warnings: report.tests.filter((t: any) => t.status === 'WARNING').length,
    ...
    return NextResponse.json({
      ...
      report: {
        ...
        summary: {
          ...
          passed: report.tests.filter((t: any) => t.status === 'PASS').length,
          failed: report.tests.filter((t: any) => t.status === 'FAIL').length,
          warnings: report.tests.filter((t: any) => t.status === 'WARNING').length,
          criticalIssues: report.vulnerabilities.filter((v: any) => v.severity === 'CRITICAL').length,
          highIssues: report.vulnerabilities.filter((v: any) => v.severity === 'HIGH').length,
          mediumIssues: report.vulnerabilities.filter((v: any) => v.severity === 'MEDIUM').length,
          lowIssues: report.vulnerabilities.filter((v: any) => v.severity === 'LOW').length
        }
      }
    });
  } catch (error) {
