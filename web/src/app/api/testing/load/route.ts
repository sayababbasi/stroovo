    ...
    return NextResponse.json({
      ...
      report: {
        ...
        summary: {
          ...
          warnings: report.warnings.length,
          criticalIssues: report.failures.filter((f: any) => f.severity === 'CRITICAL').length,
          highIssues: report.failures.filter((f: any) => f.severity === 'HIGH').length,
          mediumIssues: report.failures.filter((f: any) => f.severity === 'MEDIUM').length,
          lowIssues: report.failures.filter((f: any) => f.severity === 'LOW').length
        }
      }
    });
  } catch (error) {
