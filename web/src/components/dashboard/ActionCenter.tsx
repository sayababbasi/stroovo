...
const ActionButton = ({ title, desc, icon: Icon, color }: any) => (
    <button className="w-full flex items-center gap-4 p-4 bg-white border border-gray-100 rounded-2xl hover:border-indigo-100 hover:bg-indigo-50/20 transition-all group text-left">
        <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${ color === 'indigo' ? 'bg-indigo-50 text-indigo-600' : color === 'emerald' ? 'bg-emerald-50 text-emerald-600' : 'bg-blue-50 text-blue-600' } group-hover:scale-110 transition-transform`}>
            <Icon size={18} />
        </div>
        ...
        <div className="bg-white border border-gray-100 rounded-[32px] p-8 shadow-sm">
            <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-[10px] font-semibold text-gray-900">Smart Action Center</h3>
                </div>
            </div>
            ...
            <div className="mt-8 p-6 bg-gray-900 rounded-[24px] text-white relative overflow-hidden group cursor-pointer hover:bg-black transition-all">
                ...
                <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className="text-[10px] font-semibold">Quick Command</span>
                </div>
                <div className="text-[13px] font-semibold mb-1">Press Cmd + K</div>
