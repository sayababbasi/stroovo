...
const ProjectCard = ({ name, health, progress, probability, risk, team, color }: any) => (
    <div className="bg-white border border-gray-100 rounded-[32px] p-6 shadow-sm hover:shadow-xl hover:shadow-indigo-500/5 transition-all group">
        <div className="flex items-center justify-between mb-6">
            <div className={`p-2.5 rounded-xl ${ color === 'indigo' ? 'bg-indigo-50 text-indigo-600' : color === 'emerald' ? 'bg-emerald-50 text-emerald-600' : color === 'rose' ? 'bg-rose-50 text-rose-600' : 'bg-blue-50 text-blue-600' } group-hover:scale-110 transition-transform`}>
                <Box size={18} />
            </div>
            <div className={`px-2 py-0.5 rounded-lg text-[8px] font-semibold border ${ health === 'On Track' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : health === 'At Risk' ? 'bg-amber-50 text-amber-600 border-amber-100' : 'bg-rose-50 text-rose-600 border-rose-100' }`}>
                {health}
            </div>
        </div>

        <h3 className="text-[13px] font-semibold text-gray-900 mb-1 leading-tight group-hover:text-indigo-600 transition-colors">{name}</h3>
        <div className="flex items-center gap-2 mb-6">
            <div className="flex -space-x-2">
                ...
                <div className="w-6 h-6 rounded-full border-2 border-white bg-indigo-50 flex items-center justify-center text-[8px] font-semibold text-indigo-600">
                    +{team}
                </div>
            </div>
            <span className="text-[10px] font-bold text-gray-300">{team + 3} contributors</span>
        </div>

        <div className="space-y-4 mb-8">
            <div>
                <div className="flex items-center justify-between mb-1.5">
                    <span className="text-[9px] font-semibold text-gray-400">Execution Progress</span>
        ...
        <div className="grid grid-cols-2 gap-3 pt-6 border-t border-gray-50">
            <div className="p-3 bg-gray-50/50 rounded-2xl">
                <div className="text-[8px] font-semibold text-gray-400 mb-1 flex items-center gap-1">
                    <Brain size={10} className="text-indigo-500" />
                    Delivery Prob.
                </div>
                <div className="text-sm font-semibold text-gray-900">{probability}%</div>
            </div>
            <div className="p-3 bg-gray-50/50 rounded-2xl">
                <div className="text-[8px] font-semibold text-gray-400 mb-1 flex items-center gap-1">
        ...
        <button className="w-full mt-6 flex items-center justify-between px-4 py-3 bg-white border border-gray-100 hover:border-indigo-100 hover:bg-indigo-50/30 rounded-2xl transition-all group/btn">
            <span className="text-[10px] font-semibold text-gray-500 group-hover/btn:text-indigo-600">View Deep Audit</span>
        ...
        <section className="space-y-6">
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-[11px] font-semibold text-gray-900">Active Project Control</h3>
                </div>
                <button className="text-[10px] font-semibold text-indigo-600 hover:underline">View All Projects</button>
            </div>
