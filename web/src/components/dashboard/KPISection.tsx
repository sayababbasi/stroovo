...
const KPICard = ({ title, value, trend, icon: Icon, color, insight, prediction }: any) => (
    ...
    >
        <div className="flex items-start justify-between mb-4">
            <div className={`p-3 rounded-2xl ${ color === 'indigo' ? 'bg-indigo-50 text-indigo-600' : color === 'emerald' ? 'bg-emerald-50 text-emerald-600' : color === 'rose' ? 'bg-rose-50 text-rose-600' : color === 'amber' ? 'bg-amber-50 text-amber-600' : 'bg-blue-50 text-blue-600' } group-hover:scale-110 transition-transform`}>
                <Icon size={20} />
            </div>
        ...
        <div className="mb-6">
            <h3 className="text-[10px] font-semibold text-gray-400 mb-1">{title}</h3>
        ...
        <div className="space-y-3 pt-4 border-t border-gray-50">
            ...
            <div className="flex items-center gap-2 px-3 py-1.5 bg-gray-50 rounded-xl">
                ...
                <span className="text-[9px] font-semibold text-gray-400">Signal: {prediction}</span>
            </div>
        </div>

        {/* Decorative background element */}
        <div className={`absolute -bottom-6 -right-6 w-24 h-24 blur-3xl opacity-10 rounded-full ${ color === 'indigo' ? 'bg-indigo-500' : color === 'emerald' ? 'bg-emerald-500' : color === 'rose' ? 'bg-rose-500' : 'bg-blue-500' }`} />
    </motion.div>
);
