...
export default function EditProjectModal({ projectId, onClose, onSuccess }: EditProjectModalProps) {
    ...
    useEffect(() => {
        Promise.all([
            ...
            if (projectData && !projectData.error) {
                ...
                if (projectData.startDate) setStartDate(new Date(projectData.startDate).toISOString().split('T')[0]);
                if (projectData.endDate) setEndDate(new Date(projectData.endDate).toISOString().split('T')[0]);
                setManagerId(projectData.managerId || '');
            } else {
    ...
    if (fetching) {
        return (
            <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(9, 30, 66, 0.54)' }}>
                <div style={{ background: 'white', padding: 24, borderRadius: 12 }}>Loading project details...</div>
            </div>
        );
    }
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(9, 30, 66, 0.54)' }}>
            <div style={{ background: 'white', borderRadius: 12, width: '100%', maxWidth: 600, maxHeight: '90vh', display: 'flex', flexDirection: 'column', boxShadow: '0 8px 16px rgba(9, 30, 66, 0.12)' }}>
                ...
                <div style={{ padding: '20px 24px', borderBottom: '1px solid #DFE1E6', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <h2 style={{ margin: 0, fontSize: 20, color: '#172B4D', fontWeight: 700 }}>Edit Project</h2>
