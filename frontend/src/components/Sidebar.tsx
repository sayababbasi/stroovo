...
export default function Sidebar() {
    ...
    useEffect(() => {
        const fetchProjects = async () => {
            try {
                const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:5001'}/api/projects`, {
                    cache: 'no-store'
                });
                if (res.ok) {
                    const data = await res.json();
                    // Ensure unique projects by ID
    ...
    const recentProjects = Array.from(new Map(
        [...projects]
    ...
    const sections: NavSection[] = [
        ...
        },
        {
            ...
            items: [
                ...
                { name: 'Sprint Planning', href: '/sprint-planning', icon: Zap },
            ]
        },
        {
            title: 'Projects',
            ...
            items: [
                { name: 'All Projects', href: '/projects', icon: FolderKanban },
                {
                    name: 'Starred',
                    icon: Star,
                    children: starredProjects.length > 0
                        ? starredProjects.map(p => ({ id: p.id, name: p.name, href: `/projects?id=${p.id}` }))
                        : []
                },
                {
                    name: 'Recent',
                    ...
                    children: recentProjects.length > 0
                        ? recentProjects.map(p => ({ id: p.id, name: p.name, href: `/projects?id=${p.id}` }))
                        : []
                },
            ]
        },
        {
            ...
            collapsible: true,
            items: [
                { name: 'Engineering', href: '/teams/engineering', icon: Users },
                { name: 'Product', href: '/teams/product', icon: Users },
                { name: 'Design', href: '/teams/design', icon: Users },
            ]
        },
        {
            title: 'Collaboration',
        ...
        },
        {
            ...
            items: [
                ...
                { name: 'Smart Insights', href: '/ai/insights', icon: Lightbulb },
            ]
        },
    ];
